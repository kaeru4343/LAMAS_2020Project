...
Assume we have two agents, A<sub>1</sub> and A<sub>2</sub>, and a Kripke model M = <S,&#960,R<sub>1</sub>, R<sub>2</sub>>, where S is all of the possible states (s={s<sub>1</sub>,s<sub>2</sub>}, where s<sub>1</sub> is A<sub>1</sub>'s board and s<sub>2</sub> is A<sub>2</sub>'s board). The boards themselves can be broken down as follows: s<sub>n</sub>={s<sub>(A,1)</sub>,s<sub>(A,2)</sub>, ..., s<sub>(A,6)</sub>, s<sub>(B,1)</sub>, ..., s<sub>(G,6)</sub>}, where the x and y in s<sub>(x,y)</sub> represents the coordinates on the board.<br /> 
Furthermore, in this model, it is common knowledge that there are 3 ships. Their size is also common knowledge. So we have Ship<sub>1</sub> with length 4,Ship<sub>2</sub>, with length 3, Ship<sub>3</sub> with length 2 and from there C Ship<sub>1</sub>, C Ship<sub>2</sub>, C Ship<sub>3</sub>.<br />

Since we are monitoring two parallel representations, we also have two propositions. They are defined as follows:

p<sub>Ship</sub>(x,y) = t iff "A ship is located at coordinates (x,y) on the grid";
...
Now that we've declared our prepositions, we have &#x2112;<sup>stat</sup><sub>K</sub>(A,P), which is a union of formula and epistemic actions.
...
R<sub>2</sub>  = (s<sub>2</sub>(x,y),s<sub>2</sub>(x,y)) for all (x,y) and (s<sub>1</sub>(x,y), z<sub>1</sub>(x,y) for all z<sub>1</sub> &isin; S, which means that the relations are reflexive within thier own state and all states with the same coordinates for the othet agent's board.

<b>Actions</b>
The game consits of rounds and each round consists of one action from each agent. The agents can only do one action, which in turn will prompt at least one reaction, at most two, from the other agent. These actions and reactions will take the shape of announcements. The agent whose turn it is now will announce "&not;K<sub>i</sub> p<sub>Ship</sub>(x,y)", where i is the number of the acting agent and x and y are coordinates on the grids, which translates into "I do not know if a ship is at (x,y)". The oposing agent has to reply with the fact that there is or isn't a ship on that slot. In other words, the reply would be as follows: "&not;K<sub>i</sub> K<sub>j</sub> p<sub>Ship</sub>(x,y)" in case there is a ship there or "&not;K<sub>i</sub> K<sub>j</sub> &not;p<sub>Ship</sub>(x,y)" in case there is no ship there, where j here stands for the number of the opposing agent, which would translate into "You don't know it, but I know there is a(or no, in case not) ship at (x,y)". Assuming that all of the positions of a ship have been discovered with that announcement, agent j has to make another anouncement:  "&not;K<sub>i</sub> K<sub>j</sub> K<sub>i</sub> p<sub>Ship<sub>j,k</sub></sub>", where k is the number of the discovered ship, with respect to the ships that are common knowledge, which would roughly translate into "You don't know it, but I know that you now know the position of my ship k.".<br />
Assuming we had M &#8871; &not;K<sub>i</sub> p<sub>Ship</sub>(x,y) by applying the first reply annoncement, we get M', &not;K<sub>i</sub> K<sub>j</sub> p<sub>Ship</sub>(x,y) &#8871 C p<sub>Ship</sub>(x,y). That is the scenario when a ship is there. The same is true if a ship is not there, only for the negation of p<sub>Ship</sub>(x,y) instead.<br />
Assuming we had M &#8871; &not;K<sub>i</sub> p<sub>Ship<sub>i,j</sub></sub> by applying the second reply annoncement, we get M', &not;K<sub>i</sub> K<sub>j</sub> K<sub>i</sub> p<sub>Ship<sub>j,k</sub></sub> &#8871; C p<sub>Ship<sub>j,k</sub></sub>. <br />
Once all three ships of a player become common knowledge, the game ends.
