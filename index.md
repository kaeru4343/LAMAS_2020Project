...
R<sub>2</sub>  = (s<sub>2</sub>(x,y),s<sub>2</sub>(x,y)) for all (x,y) and (s<sub>1</sub>(x,y), z<sub>1</sub>(x,y) for all z<sub>1</sub> &isin; S, which means that the relations are reflexive within their own state and all states with the same coordinates for the other agent's board.

<b>Actions</b>

The game consists of rounds and each round consists of one action from each agent. The agents can only do one action, which in turn will prompt at least one reaction, at most two, from the other agent. These actions and reactions will take the shape of announcements. The agent whose turn it is now will announce "&not;K<sub>i</sub> p<sub>Ship</sub>(x,y)", where i is the number of the acting agent and x and y are coordinates on the grids, which translates into "I do not know if a ship is at (x,y)". The opposing agent has to reply with the fact that there is or isn't a ship on that slot. In other words, the reply would be as follows: "&not;K<sub>i</sub> K<sub>j</sub> p<sub>Ship</sub>(x,y)" in case there is a ship there or "&not;K<sub>i</sub> K<sub>j</sub> &not;p<sub>Ship</sub>(x,y)" in case there is no ship there, where j here stands for the number of the opposing agent, which would translate into "You don't know it, but I know there is a(or no, in case not) ship at (x,y)". Assuming that all of the positions of a ship have been discovered with that announcement, agent j has to make another announcement:  "&not;K<sub>i</sub> K<sub>j</sub> K<sub>i</sub> p<sub>Ship<sub>j,k</sub></sub>", where k is the number of the discovered ship, with respect to the ships that are common knowledge, which would roughly translate into "You don't know it, but I know that you now know the position of my ship k.".<br />
Assuming we had M &#8871; &not;K<sub>i</sub> p<sub>Ship</sub>(x,y) by applying the first reply announcement, we get M', &not;K<sub>i</sub> K<sub>j</sub> p<sub>Ship</sub>(x,y) &#8871 C p<sub>Ship</sub>(x,y). That is the scenario when a ship is there. The same is true if a ship is not there, only for the negation of p<sub>Ship</sub>(x,y) instead.<br />
Assuming we had M &#8871; &not;K<sub>i</sub> p<sub>Ship<sub>i,j</sub></sub> by applying the second reply announcement, we get M', &not;K<sub>i</sub> K<sub>j</sub> K<sub>i</sub> p<sub>Ship<sub>j,k</sub></sub> &#8871; C p<sub>Ship<sub>j,k</sub></sub>. <br />
Again, once all three ships of a player become common knowledge, the game ends.
